		...
		{#each source as { id, category, icon, name, description, price, kg }}
			...
			>
				<a href="{id}" class="group flex flex-col h-full">
					...
					<h1 class="mt-5 justify-between flex">
						<div class="max-w-[200px]">{name}</div>
						<div class="flex flex-wrap">${price}</div>
					</h1>

					{#if kg}
						<p class="mt-2">Weight: {kg}kg.</p>
					{/if}

					<h2 class="text-sm mt-5  line-clamp-3 flex-grow">
						{description}
					</h2>

					<div
						class="flex justify-between items-end flex items-center mt-5 transition-colors group-hover:text-red-900"
					>
